...
_Run-that-app_ executes CLI applications on Linux, Windows,
macOS, and BSD without the need to install them first.

Run-that-app is standalone, minimalistic, and completely non-invasive. No
magic, no environment or configuration changes, no environment variables to set,
no application shims or stubs, no shell integrations or other trickery, no
dependencies, no plugins, no application repository, no Docker, no emulation, no WASM. All you do is call a single
