...
fn display_verbose(event: Event) {
    match event {
        ...
        Event::DownloadBegin { app, url } => println!("download {} from {}", app, url.cyan().bold()),
        Event::DownloadSuccess => println!("finished download"),
        Event::DownloadFail { code } => println!("download failed: {}", code.to_string().red().bold()),
        Event::NotOnline => todo!(),
        Event::DownloadNotFound => todo!(),
        Event::CompileGoStart { go_path, args } => println!("{go_path} {}", args.join(" ")),
        Event::CompileGoSuccess => todo!(),
        Event::CompileRustStart { cargo_path, args } => println!("{} {}", cargo_path.to_string_lossy(), args.join(" ")),
