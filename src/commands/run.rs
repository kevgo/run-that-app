use crate::applications::{self, AnalyzeResult, AppDefinition, ApplicationName, Apps};
...
fn load_from_path(app_to_run: &dyn AppDefinition, range: &semver::VersionReq, ctx: &RuntimeContext) -> Result<Option<ExecutableCallDefinition>> {
  let (app_to_install, executable_name, executable_args) = applications::carrier(app_to_run, &Version::from(""), ctx.platform);
...
fn load_or_install_from_yard(
  ...
  let (app_to_install, executable_name, executable_args) = applications::carrier(app_definition, version, ctx.platform);
