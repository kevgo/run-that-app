...
pub fn log(event: Event) {
    ...
    match event {
        Event::AnalyzeExecutableQuery { cmd, args } => eprintln!("{}", format!("{cmd} {}", args.join(" ")).bold()),
        ...
        Event::YardCheckForExistingApp { path } => eprintf!("Checking for existing app {}", path.to_string_lossy()),
    }
}
